...
> 상태: **구현 완료** — PR1~PR4 전부 (2026-08-26, #701·#702·#703·PR4). 상위 문서 [`improvement-roadmap-2026-08.md`](improvement-roadmap-2026-08.md) §9.
...
- OpenAPI 스펙을 `docs/api/`에 **생성**한다(PR3). ~~수기 또는~~ — 손으로 쓴 스펙은
  반드시 드리프트하고, **공개 문서의 거짓말은 없는 문서보다 나쁘다**(읽는 쪽이 계약으로
  믿는다). 경로·스코프·한도는 허용목록에서 생성하고 사람은 설명만 쓴다. 설명이 없는
  공개 경로가 있으면 생성기가 실패하고, 재생성을 잊으면 유닛이 실패한다.
- **응답 스키마는 담지 않는다**(PR3 결정, §7 결정 10). 22개 엔드포인트의 응답 형태를
  손으로 유지하는 것은 지킬 수 없는 약속이다.

### 3.3 MCP 서버

- 신규 패키지 `apps/mcp` — stdio 전송, PAT로 공개 API를 호출하는 **얇은 래퍼**. 도메인 로직 없음.
  - 도구 **11종**. 공개 표면 22개를 전부 도구로 만들지 않는다 — 도구가 많으면 LLM이
    고르기 어려워지고, 나머지는 앱 화면용 부트스트랩이라 대화에 쓸모가 없다.
  - **세션 토큰을 넣으면 거부한다**(PR4 추가 안전장치). 세션 토큰은 공개 표면 제한을
    받지 않아 계정 삭제까지 열린다 — 접두사로 걸러 낸다.
  - 오류를 예외가 아니라 `isError` + **고칠 수 있는 문장**으로 돌려준다. LLM이 읽고
    스스로 고칠 수 있어야 한다.
...
| **3** | `docs(api): OpenAPI 스펙을 게시한다` | `docs/api/` 스펙 **생성**(수기 아님) + 멱등성·rate limit 명시 + 드리프트 유닛 | 낮 | 스펙↔표면 일치 |
...
10. **OpenAPI는 생성물, 응답 스키마는 제외** (PR3 결정). 경로·스코프·한도는 코드에서
    생성해 드리프트를 유닛으로 막고, 응답 본문은 문서화하지 않는다 — 유지할 수 없는
    약속을 하지 않는 편이 정직하다. 계약 산문은 `docs/api/README.md`에 둔다.
