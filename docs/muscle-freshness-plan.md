...
> 상태: **구현 완료** — PR1~PR3 전부 (2026-08-25, #698·#699·PR3). 상위 문서 [`improvement-roadmap-2026-08.md`](improvement-roadmap-2026-08.md) §7.
...
6. **`capacityKg === 0`은 "100% 신선"이 아니라 "기록 없음"** (2026-08-25 신설) → 모델은 둘 다 `freshnessPct: 100`을 주지만 `capacityKg`로 구분된다. prod에서 `Core`가 이 상태라 **전 화면에 상시 노출되는 케이스**다. 게이지를 채워 "회복 완료"로 읽히게 두면 거짓말이 된다 — PR2(#699)에서 별도 표기했다.
7. **표시 위치 → 통계 덱** (PR2에서 확정) → §3.3이 "홈 Today 덱 또는 통계 덱"으로 열어 뒀다. 신선도는 **처방이 아니라 조언**이고(§비목표), 계획서 자신이 §3.3 말미에서 "로깅 화면이 이미 조밀하다"는 이유로 배지를 뺐다 — 같은 논리가 Today 덱에도 적용된다. 통계 덱에는 파생 지표 카드 가족이 이미 있다.
8. **회복 시간 조정은 근거 시트 안에만 둔다** (PR3에서 확정) → 설정 화면에 별도 항목을 만들지 않는다. 이 값은 **모델의 가정**이라 식과 기여 세션 옆에 있어야 의미가 있고, 설정 목록에 숫자 하나로 떼어 놓으면 무엇을 바꾸는지 알 수 없다.
9. **눈금 5개(4~8일), 연속 값 아님** (PR3에서 확정) → 슬라이더로 137시간 같은 값을 만들게 하면 정밀해 보이는 착각을 준다. 가정도 눈금으로 두는 편이 정직하다. 정규화는 **가장 가까운 눈금으로 스냅**한다 — 범위 밖 값을 기본값으로 되돌리면 사용자 의도가 사라진다.
