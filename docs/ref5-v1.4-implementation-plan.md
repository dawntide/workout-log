...
| 기존 e2e 여정 | 카드 이름만 교체 | **OAP 기본 결과를 HOLD로** | 계획서는 기존 여정이 무영향이라고 봤다. 실제로는 세션을 반복하는 동안 OAP가 PASS로 채워져 사다리가 4단까지 올라가고, 네거티브가 붙어 세션이 10→11세트가 됐다. OAP를 검사하지 않는 여정은 중립(HOLD)을 기본값으로 둬 사다리를 멈춰 세운다 |
| e2e 스쿼트 기대값 | 그대로 | **선택 인자로** | 하드 밀도(§9)가 세션 간격으로 정해지므로, 스쿼트를 검사하지 않는 새 여정이 그 순서를 미리 계산할 이유가 없다. 틀린 추측이 실제로 nightly를 깨뜨렸다 |
