...
닫기 버튼은 **항상** 존재한다 — 프리미티브가 강제한다:

| 변형 | 헤더 | 닫기 위치 | 용도 |
|------|------|-----------|------|
| 기본 | `title`(+`description`) | 우측 X | 정보·선택·확인 시트 |
| 액션 헤더 | `primaryAction` 지정 | **좌측 X + 우측 ✓** | 편집/저장이 있는 시트 |
| headless / 커스텀 `header` | 자유 배치 | 플로팅 X(우상단, 자동) | 페이지형 콘텐츠 |
