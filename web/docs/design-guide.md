...
#### 시트 레이아웃 규칙 (Hard Rule)

닫기 버튼은 **항상 존재하고, 항상 좌상단(패널 좌측 16px)** 이다 — 프리미티브가 강제한다. 사용자는 위치를 외워서 누르므로 변형마다 자리가 달라지면 안 된다. 우측은 액션 헤더의 ✓가 쓰므로, 통일 가능한 자리는 좌측뿐이다.

| 변형 | 헤더 | 우측 | 용도 |
|------|------|------|------|
| 기본 | `title`(+`description`) | (빈 칸) | 정보·선택·확인 시트 |
| 액션 헤더 | `primaryAction` 지정 | ✓ | 편집/저장이 있는 시트 |
| headless | 자유 배치 | (빈 칸) | 페이지형 콘텐츠 |

- headless의 X는 드래그 핸들 행의 **좌측 칸을 실제로 차지한다**(`--lead`). absolute로 띄우면 본문이 그 밑으로 파고든다 — 실제로 긴 eyebrow가 겹쳤다.
- 커스텀 `header`로 헤더를 통째로 갈아끼우지 말 것. 제목 블록이 필요하면 **본문 첫 블록**으로 넣는다(프로그램 상세 시트가 그 예). 헤더를 대체하면 닫기 위치 통일이 깨진다.

- **편집(저장·적용·확인)이 있는 시트는 `primaryAction`(헤더 ✓)** 을 쓴다. 본문 하단 풀폭 저장 버튼으로 재구현하지 말 것 (설정 모달·플랜 관리·커스터마이징 등 전부 동일).
- 예외 두 갈래: **확인 다이얼로그**(삭제 확인 등)와 **진입 CTA**(프로그램 시작·필터 결과 보기)는 `footer` 풀폭 버튼 — 이 둘은 헤더 ✓를 쓰지 않는다.
- 커스텀 `header`는 우상단 `--v2-s-8` 폭을 비워 플로팅 X와 겹치지 않게 한다.
- ⚠️ **`description`은 현재 화면에 렌더되지 않는다** — `MINIMAL_COPY_MODE = true`가 전역으로 억제한다([`minimal-copy.ts`](../src/lib/ui/minimal-copy.ts)). 보조 문구에만 쓰고, **없으면 사용자가 판단을 그르치는 정보**(예: "다른 기기가 로그아웃됩니다")는 반드시 본문에 둔다.
- headless 본문 헤더는 `v2-eyebrow` + `h1`(`.v2-h1`, 크기 오버라이드 금지) 조합을 따른다. `.v2-h1`/`.v2-h2`는 이미 display 폰트라 `v2-font-display`를 덧붙이지 않는다.

#### 확인 다이얼로그 구조 (Hard Rule)

삭제·차단 안내 등 확인 시트는 **한 가지 골격**만 쓴다 (`app-dialog-provider`가 기준):

```
헤더: title (기본 헤더, 가운데 정렬은 CSS가 처리 — 본문에 제목을 다시 그리지 말 것)
본문: <V2Card tone="danger" | "inset"> 안에 메시지 한 단락
footer: V2PrimaryBtn(주 행동) + V2SecondaryBtn(취소) 세로 스택
```
...
- **최소 터치 타겟**: 44×44px (`var(--touch-target)`)
  - 규칙의 기준은 **누를 수 있는 영역**이지 보이는 크기가 아니다. 아이콘 버튼처럼 작은 실루엣이 의도된 곳은 실루엣을 키우지 말고 `.v2-tap-44`를 붙여 투명 `::after`로 히트 영역만 44×44로 넓힌다(호스트에 `position`이 잡혀 있어야 한다).
  - 카드 전체가 하나의 대상인 패턴은 `.program-card-title-btn`처럼 `::after { inset: 0 }` 스트레치 오버레이를 쓴다 — 이 경우 요소 자체 박스는 작아도 위반이 아니다.
