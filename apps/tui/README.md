...
> **상태**: 가입 → 로깅(RPE) → 통계(e1RM·주간 볼륨) → 기록(편집) → 플랜 → 운동 CRUD → 설정·계정 → 백업/복원까지 **전 워크플로가 TUI만으로 동작**. REF5 v1.4의 미리보기·시작·재개·기록·상태 확인도 지원합니다. helix식 모달 앱(6버퍼).
...
- **today** — `i` 편집 · `e` 운동 추가 · `x` 완료 · `o` 세트 추가 · `d` 삭제 · `s` 저장 · `h`/`l` 셀 이동(무게/reps/`@`rpe). 서버 e1RM/PR 표시 · 플랜 세션 로딩.
...
- **today / REF5** — SQ 하드 6회, BP·PULL 집중 4회, DL·OHP 4회의 기본 판정창 진행/기준/완료 횟수와 판정 의미를 항상 표시합니다. v1.4부터 OAP 스킬 슬롯의 좌/우 현재 단과 승급 연속도 함께 표시합니다. 플랜 시간대 기준 실제 첫 SQ 시작 시각과 체중·MICRO 입력 → (BP 집중 차례면 OAP 되돌리기 여부) → 쓰기 없는 미리보기 → 시작 확정. 시작 후 처방은 잠기며 `i`/`⏎` reps · `x` 계획 reps · `t` 종료 사유 · `s` 판정 검토/저장. 저장 후 판정창을 자동 갱신하고, 미완료 생성 세션은 다시 접속해 이어갈 수 있습니다.
- **stats** — `v`로 4개 뷰 순환(e1RM 추이 → 주간 볼륨 → 부위별 신선도 → 체중 추이) · `jk` 운동 · `[ ]` 범위 · `b` 차트 스타일 · `a` 체중 기록.
- **history** — 히트맵 + 세션 리스트 · `⏎` 상세 · `e` 편집(today 재사용) · `d` 삭제.
- **programs / REF5** — 템플릿 생성 시 IANA 시간대 → 최근 기록/추정 1RM 추천 또는 고급 직접 입력 → 첫 처방 확인. e1RM은 저장하지 않고 확정된 직접 kg만 계획에 남깁니다. OAP 시작 단은 서버 기본값(좌/우 2단)을 씁니다 — 단을 골라 시작하려면 웹에서 계획을 만드세요. 모든 플랜에서 `v`로 진행 상태를 엽니다 — REF5는 엔진 상태(NEXT/표준/윈도/LOCK/MICRO), 그 외는 판정 이력. `R` 새로고침.
- **settings** — 환경설정 행은 `⏎` 순환 / `i` 숫자 편집. ACCOUNT 섹션에서 이메일 인증·세션 종료·비밀번호·계정 삭제, 그리고 **액세스 토큰**(`n` 발급 · `d` 폐기 · 발급 직후 `y` 클립보드 복사 — OSC 52라 SSH에서도 됩니다). 발급한 토큰은 MCP 서버·백업 스크립트가 쓰며, 평문은 발급 화면에서만 보입니다.
