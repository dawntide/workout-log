...
- **programs / REF5** — 템플릿 생성 시 IANA 시간대 → 최근 기록/추정 1RM 추천 또는 고급 직접 입력 → 첫 처방 확인. e1RM은 저장하지 않고 확정된 직접 kg만 계획에 남깁니다. 모든 플랜에서 `v`로 진행 상태를 엽니다 — REF5는 엔진 상태(NEXT/표준/윈도/LOCK/MICRO), 그 외는 판정 이력. `R` 새로고침.
