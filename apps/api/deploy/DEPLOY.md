...
- 에러 추적: **도입하지 않는다**(2026-08-11 결정, 아래 참조).
- 헬스: `GET /health`(무인증) — 업타임 모니터 대상.

---
## 7. 하지 않기로 한 것 (2026-08-11)

아래 넷은 오래 "미해결(인프라 결정 필요)"로 올라와 있었다. 결정을 내렸으니 **백로그에서 내린다**.
다시 열려면 각 항목의 조건이 실제로 성립해야 한다 — 조건 없이 재론하지 말 것.

| 항목 | 결정 | 다시 열 조건 |
|---|---|---|
| 에러 추적(Sentry 등) | **안 한다.** 운영 인프라를 늘리는 비용이 이 규모(유저 2명·1인 운영)에서 얻는 값보다 크다. 지금도 실패는 보인다 — 배포 검증 실패는 이슈로 승격되고(#669), 서버 에러는 `api.request` 구조화 로그와 Supabase 로그에 남는다 | 재현 안 되는 클라이언트 에러를 쫓게 되거나, Vercel 런타임 로그 1시간 보존이 조사에 부족해질 때 |
| CORS 정책 | **성립하지 않는다.** 현행은 인프로세스 모드라 브라우저가 apps/api를 직접 호출하지 않는다(same-origin `/api/*` → catch-all) | apps/api를 별도 도메인으로 재분리하고 **브라우저가 그 도메인을 직접** 부르게 될 때. allowed origins + `Allow-Credentials`가 그때 필요하다 |
| 시크릿 중앙관리(Doppler/1Password 등) | **안 한다.** 1인 운영에 배포처 2곳(Vercel env·VPS `.env`)이라 도구 도입이 과잉 | 시크릿을 만지는 사람이 늘거나 배포처가 더 늘 때 |
| TUI fleet 버전 추적 | **안 한다.** 설치 대상이 사실상 소유자 본인이고, `ironlog --version`으로 확인된다 | TUI 사용자가 소유자 밖으로 늘어 "누가 어느 버전을 쓰는지"가 실제 질문이 될 때 |

자세한 배경은 다중-프론트/단일-백엔드 리서치 리포트 참조.
